...
function atmosphere_setup() {
	...
	define( 'HEADER_IMAGE_WIDTH', apply_filters( 'atmosphere_header_image_width', 794 ) );
	define( 'HEADER_IMAGE_HEIGHT', apply_filters( 'atmosphere_header_image_height', 138 ) );
...
function atmosphere_posted_in() {
	...
	if ( $tag_list ) {
		...
		$posted_in = __( 'Posted in %1$s ¥ <a href="%3$s" title="Permalink to %4$s" rel="bookmark">Permalink</a>', 'atmosphere' );
	} else {
		$posted_in = __( '<a href="%3$s" title="Permalink to %4$s" rel="bookmark">Permalink</a>', 'atmosphere' );
	}
	printf(
		$posted_in,
...
return preg_replace('/>Home</', '>Games<', $menu_text, 1);
}
